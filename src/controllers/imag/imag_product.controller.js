...
export const postImagProduct = async (req, res) => {
  try {
    ...
    //Se comprueba si ya el producto tiene una imagen
    const search_imag = await SearchImagProductId(id_produ);
    if (search_imag > 0) {
      //Se borra la imagen en la ruta publica
      await fs.unlink(req.file.path);
      return res.send({
        status: "mal",
        description: "El producto ya cuenta con una imagen",
      });
    }
    ...
    //se invoca el servicio para registrar una imagen de un producto
