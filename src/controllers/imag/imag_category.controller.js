...
import {
  SearchCategoryId,
  RegisterImagCategory,
  SearchImagCategoryId,
  ReturnImagCategoryId_public,
  DeleteImagCategoryId,
} from "../../services/imag/imag_category.services.js";
...
export const postImagCategory = async (req, res) => {
  try {
    ...
    //Se comprueba si ya la categoria tiene una imagen
    const search_imag = await SearchImagCategoryId(id_categ);
    if (search_imag > 0) {
      //Se borra la imagen en la ruta publica
      await fs.unlink(req.file.path);
      return res.send({
        status: "mal",
        description: "La categoria ya cuenta con una imagen",
      });
    }
