import {
  getAllProduct, //Servicio que devuelve todos los productos
...
export const getProduct = async (req, res) => {
  try {
    ...
    if (pro == 0) {
      res.send({
        status: "ok",
        description: "Lista de productos se encuentra vacia",
      });
    } else {
      res.send({ status: "ok", description: "Lista de productos", data: pro });
    }
  } catch (error) {
    console.log(error);
  }
...
export const getProductForIdCategory = async (req, res) => {
  try {
    ...
    //se invoca el servicio que devuelve todos los productos por un id
    ...
    if (prod == 0) {
      ...
      res.send({ status: "ok", description: "Lista de productos", data: prod });
    }
  } catch (error) {
    console.log(error);
  }
...
export const postProduct = async (req, res) => {
  try {
    ...
    if (searchPro > 0) {
      return res.send({ status: "mal", description: "Producto ya registrada" });
    }
