...
import {
  getAllBuyForUser, //Servicio que devuelve todas las compras del usuario que inicio seccion
...
export const getBuy = async (req, res) => {
  try {
    ...
    const allbuyUser = await getAllBuyForUser(decodificada.id);

    if (allbuyUser == 0) {
      res.send({
        status: "ok",
        description: "La lista de compra se encuentra vacia",
      });
    } else {
      res.send({
        status: "ok",
...
export const getDetail_Buy = async (req, res) => {
  try {
    ...
    if (search_buy == 0) {
      return res.send({ status: "mal", description: "Compra no registrada" });
    }
