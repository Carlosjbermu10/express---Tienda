...
export const SearchImagCategoryId = async (id_categ) => {
  const [rows] = await pool.query(
    "SELECT * FROM imag_category WHERE id_category = ?",
    [id_categ]
  );
  return rows.length;
};
...
export const ReturnImagCategoryId_public = async (id_imag_categ) => {
  const [rows] = await pool.query(
    "SELECT * FROM imag_category WHERE id_imag_category = ?",
    [id_imag_categ]
  );
...
export const RegisterImagCategory = async (imag_cate) => {
  ...
  const [rows] = await pool.query(
    "INSERT INTO imag_category (id_category, url_imag, id_public) VALUES(?,?,?)",
...
export const DeleteImagCategoryId = async (id_imag_categ) => {
  const [rows] = await pool.query(
    "DELETE FROM imag_category WHERE id_imag_category = ?",
    [id_imag_categ]
  );
  return rows;
};
