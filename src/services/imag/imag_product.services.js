...
export const SearchImagProductId = async (id_imag_prod) => {

    const [rows] = await pool.query
    ('SELECT * FROM imag_product WHERE id_imag_product = ?', [id_imag_prod])
    return rows.length
...
export const RegisterImagProduct = async (imag_prod) => {
    ...
    const [rows] = await pool.query('INSERT INTO imag_product (id_product, url_imag, id_public) VALUES(?,?,?)',
...
export const DeleteImagProductId = async (id_imag_prod) => {
    ...
    const [rows] = await pool.query
    ('DELETE FROM imag_product WHERE id_imag_product = ?', [id_imag_prod])
    return rows
